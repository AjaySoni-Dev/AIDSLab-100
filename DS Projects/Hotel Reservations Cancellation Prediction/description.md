...
This project is a clean, student-friendly implementation of a **booking cancellation prediction**. The main goal is to predict whether a hotel reservation may be cancelled. The notebook keeps the workflow simple: load the dataset, understand the columns, clean the data, train a baseline model, and check how well the model performs.
...
A raw dataset is useful only when it is converted into a clear decision-making workflow. In this project, the data is processed step by step so that important patterns can be found and a machine learning model can be trained. The expected output is based on `no_of_previous_cancellations`.

## Files Included

| File | Purpose | Quick Note |
|---|---|---|
| `Hotel Reservations.csv` | Main data source for this folder | 2000 sampled rows x 19 columns |
...
| Column | Role |
|---|---|
| `Booking_ID` | Input feature used in the modelling workflow |
| `no_of_adults` | Input feature used in the modelling workflow |
| `no_of_children` | Input feature used in the modelling workflow |
| `no_of_weekend_nights` | Input feature used in the modelling workflow |
| `no_of_week_nights` | Input feature used in the modelling workflow |
| `type_of_meal_plan` | Input feature used in the modelling workflow |
| `required_car_parking_space` | Input feature used in the modelling workflow |
| `room_type_reserved` | Input feature used in the modelling workflow |
| `lead_time` | Input feature used in the modelling workflow |
| `arrival_year` | Input feature used in the modelling workflow |
| `arrival_month` | Input feature used in the modelling workflow |
| `arrival_date` | Input feature used in the modelling workflow |
