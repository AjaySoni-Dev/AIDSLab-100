...
This project is a clean, student-friendly implementation of a **medical cost regression**. The main goal is to predict medical insurance charges from profile and health fields. The notebook keeps the workflow simple: load the dataset, understand the columns, clean the data, train a baseline model, and check how well the model performs.
...
A raw dataset is useful only when it is converted into a clear decision-making workflow. In this project, the data is processed step by step so that important patterns can be found and a machine learning model can be trained. The expected output is based on `charges`.

## Files Included

| File | Purpose | Quick Note |
|---|---|---|
| `insurance.csv` | Main data source for this folder | 1338 sampled rows x 7 columns |
...
| `sex` | Input feature used in the modelling workflow |
| `bmi` | Input feature used in the modelling workflow |
| `children` | Input feature used in the modelling workflow |
| `smoker` | Input feature used in the modelling workflow |
| `region` | Input feature used in the modelling workflow |
| `charges` | Target / output column used in the modelling workflow |
