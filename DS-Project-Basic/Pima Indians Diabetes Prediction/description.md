...
This project is a clean, student-friendly implementation of a **diabetes classification**. The main goal is to predict diabetes risk from clinical measurements. The notebook keeps the workflow simple: load the dataset, understand the columns, clean the data, train a baseline model, and check how well the model performs.
...
A raw dataset is useful only when it is converted into a clear decision-making workflow. In this project, the data is processed step by step so that important patterns can be found and a machine learning model can be trained. The expected output is based on `Outcome`.

## Files Included

| File | Purpose | Quick Note |
|---|---|---|
| `diabetes.csv` | Main data source for this folder | 768 sampled rows x 9 columns |
...
| Column | Role |
|---|---|
| `Pregnancies` | Input feature used in the modelling workflow |
| `Glucose` | Input feature used in the modelling workflow |
| `BloodPressure` | Input feature used in the modelling workflow |
| `SkinThickness` | Input feature used in the modelling workflow |
| `Insulin` | Input feature used in the modelling workflow |
| `BMI` | Input feature used in the modelling workflow |
| `DiabetesPedigreeFunction` | Input feature used in the modelling workflow |
| `Age` | Input feature used in the modelling workflow |
| `Outcome` | Target / output column used in the modelling workflow |
