...
This project is a clean, student-friendly implementation of a **delivery performance classification**. The main goal is to predict whether an e-commerce order will reach on time. The notebook keeps the workflow simple: load the dataset, understand the columns, clean the data, train a baseline model, and check how well the model performs.
...
A raw dataset is useful only when it is converted into a clear decision-making workflow. In this project, the data is processed step by step so that important patterns can be found and a machine learning model can be trained. The expected output is based on `Reached.on.Time_Y.N`.

## Files Included

| File | Purpose | Quick Note |
|---|---|---|
| `E_Commerce.csv` | Main data source for this folder | 2000 sampled rows x 12 columns |
...
| `Warehouse_block` | Input feature used in the modelling workflow |
| `Mode_of_Shipment` | Input feature used in the modelling workflow |
| `Customer_care_calls` | Input feature used in the modelling workflow |
| `Customer_rating` | Input feature used in the modelling workflow |
| `Cost_of_the_Product` | Input feature used in the modelling workflow |
| `Prior_purchases` | Input feature used in the modelling workflow |
| `Product_importance` | Input feature used in the modelling workflow |
| `Gender` | Input feature used in the modelling workflow |
| `Discount_offered` | Input feature used in the modelling workflow |
| `Weight_in_gms` | Input feature used in the modelling workflow |
| `Reached.on.Time_Y.N` | Target / output column used in the modelling workflow |
